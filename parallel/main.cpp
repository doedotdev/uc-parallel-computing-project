...
void cu_edge_detection(const uchar4 * const h_inputImageRGBA, uchar4 * const d_inputImageRGBA,
                        ...
                        const int filterWidth, unsigned char*);
...
int main(int argc, char **argv) {
  ...
  unsigned char h_finalImg[numRows() * numCols()];
  memset(h_finalImg, 0, sizeof(unsigned char) * numPixels);
  allocateMemoryAndCopyToGPU(numRows(), numCols(), h_filter, filterWidth);
  GpuTimer timer;
  timer.Start();
  cu_edge_detection(h_inputImageRGBA, d_inputImageRGBA, d_outputImageRGBA, numRows(), numCols(), filterWidth, h_finalImg);
  timer.Stop();
  ...
  postProcess(output_file, h_finalImg);
  cleanUp();
